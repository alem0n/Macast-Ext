...
class MPVRenderer(Renderer):
    ...
    def __init__(self, lang=gettext.gettext, path="mpv"):
        ...
        if os.name == 'nt':
            self.mpv_sock = r"\\.\pipe\macast_mpvsocket{}".format(mpv_rand)
        else:
        ...
        logger.info("MPVRenderer init: path=%s, pipe=%s", self.path, self.mpv_sock)
    ...
    def set_media_url(self, url, start="0"):
        """ data : string
        """
        logger.info("set_media_url called: url=%s, start=%s, ipc_connected=%s",
                    url, start, self.ipc_once_connected)
        options = {'start': start}
        ...
        cmd = ['loadfile', url, 'replace', 0,
               ','.join([f'{i}={options[i]}' for i in options])]
        logger.info("sending loadfile command: %s", cmd)
    ...
    def update_state(self, res):
        ...
        """
        res = json.loads(res)
        if 'error' in res and res['error'] != 'success':
            logger.error("mpv error response: %s", res)
            return
        if 'id' in res:
    ...
    def send_command(self, command):
        ...
        """
        logger.info("send_command: %s", str(command)[:200])
        if self.ipc_sock is None:
            logger.error("send_command: ipc_sock is None, command dropped: %s", str(command)[:200])
            return False
        ...
        with self.command_lock:
            try:
                ...
                logger.error('sendCommand error: %s', str(e))
                return False
    ...
    def start_ipc(self):
        ...
        logger.info("start_ipc: connecting to %s", self.mpv_sock)
        while self.ipc_running and self.running and self.mpv_thread.is_alive():
            try:
                time.sleep(0.5)
                logger.info("mpv ipc attempting connection to: %s", self.mpv_sock)
                if os.name == 'nt':
                ...
                logger.error("mpv ipc connect failed: %s", str(e)[:200])
                continue
            res = b''
            msgs = None
            while self.ipc_running:
                try:
                    ...
                    if data == b'':
                        logger.info("mpv ipc: received empty data, breaking")
                        break
                    res += data
                    if data[-1] != 10:
                        continue
                except Exception as e:
                    logger.info("mpv ipc recv error: %s", str(e)[:200])
                    break
                try:
                    ...
                    logger.error("decode error: %s", str(e)[:200])
                    logger.error("decode error data: %s", repr(msg)[:200])
                finally:
                    res = b''
            self.ipc_sock.close()
    ...
    def start_mpv(self):
        ...
        while self.running and error_time > 0:
            ...
            # start mpv
            logger.info("mpv starting: %s", ' '.join(params[:5]))
            ...
            try:
                ...
                logger.info("mpv process started, pid=%s", self.proc.pid)
                _, stderr = self.proc.communicate()
                if stderr:
                    logger.info("mpv stderr (last 500): %s", stderr.decode(errors='replace')[-500:])
            except FileNotFoundError as e:
                logger.error("mpv executable not found at path: %s", self.path)
            except Exception as e:
                logger.error("mpv start exception: %s", str(e))
            logger.info("mpv stopped (exit code: %s)", self.proc.returncode if self.proc else 'N/A')
            if self.running and not self.ipc_once_connected:
                # There should be a problem with the MPV startup parameters
                time.sleep(1)
                error_time -= 1
                logger.error("mpv restarting (remaining attempts: %s)", error_time)
        if error_time <= 0:
    ...
    def start(self):
        ...
        logger.info("MPVRenderer.start() called — launching mpv and ipc threads")
        self.mpv_thread = threading.Thread(target=self.start_mpv, name="MPV_THREAD")
        self.mpv_thread.daemon = True
